...
<div class="card mb-4">
    ...
    <div class="card-body">
        ...
        @else
            <div class="accordion" id="insp-accordion-{{ $selectedId }}">
                @php $firstGroup = true; @endphp
                @foreach ($describedProps as $groupName => $groupDefinitions)
                    @php
                        // wire:ignore.self di tombol dan panel di bawah ini sengaja
                        // dipasang supaya status buka/tutup accordion (kelas "show"
                        // Bootstrap, di-toggle murni lewat JS klien) tidak ditimpa balik
                        // ke bawaan tiap kali Livewire me-render ulang inspektor (mis.
                        // setelah field lain selesai diketik) — tanpa ini panel akan
                        // "menutup sendiri" setiap ada perubahan lain di form.
                        $groupId = 'insp-group-'.md5($selectedId.$groupName);
                        $isOpen = $firstGroup;
                        $firstGroup = false;
                    @endphp
                    <div class="accordion-item">
                        <h2 class="accordion-header">
                            <button type="button" class="accordion-button{{ $isOpen ? '' : ' collapsed' }}"
                                ...
                                data-bs-target="#{{ $groupId }}" aria-expanded="{{ $isOpen ? 'true' : 'false' }}"
                                aria-controls="{{ $groupId }}">
                                {{ $groupName }}
                            </button>
                        </h2>
                        <div id="{{ $groupId }}" class="accordion-collapse collapse{{ $isOpen ? ' show' : '' }}" wire:ignore.self>
